...
class Dataset_mekong(Dataset):
    ...
    def __read_data__(self):
        ...
        # cols.remove('Timestamp (UTC+07:00)')
        df_raw = df_raw[['Timestamp (UTC+07:00)'] + [self.target]]
        ...
        df_stamp = df_raw[['Timestamp (UTC+07:00)']][border1:border2]
        df_stamp['Timestamp (UTC+07:00)'] = pd.to_datetime(df_stamp['Timestamp (UTC+07:00)'])
        if self.timeenc == 0:
            df_stamp['month'] = df_stamp['Timestamp (UTC+07:00)'].apply(lambda row: row.month, 1)
            df_stamp['day'] = df_stamp['Timestamp (UTC+07:00)'].apply(lambda row: row.day, 1)
            df_stamp['weekday'] = df_stamp['Timestamp (UTC+07:00)'].apply(lambda row: row.weekday(), 1)
            df_stamp['hour'] = df_stamp['Timestamp (UTC+07:00)'].apply(lambda row: row.hour, 1)
            data_stamp = df_stamp.drop(['Timestamp (UTC+07:00)'], 1).values
        elif self.timeenc == 1:
            data_stamp = time_features(pd.to_datetime(df_stamp['Timestamp (UTC+07:00)'].values), freq=self.freq)
...
class Dataset_mekong_cross(Dataset):
    ...
    def __read_data__(self):
        ...
        # cols.remove(self.target)
        # cols.remove('Timestamp (UTC+07:00)')
        df_raw1 = df_raw1[['Timestamp (UTC+07:00)'] + [self.target]]
        df_raw2 = df_raw2[['Timestamp (UTC+07:00)'] + [self.target]]
        ...
        df_stamp = df_raw1[['Timestamp (UTC+07:00)']][border1:border2]
        df_stamp['Timestamp (UTC+07:00)'] = pd.to_datetime(df_stamp['Timestamp (UTC+07:00)'])
        if self.timeenc == 0:
            df_stamp['month'] = df_stamp['Timestamp (UTC+07:00)'].apply(lambda row: row.month, 1)
            df_stamp['day'] = df_stamp['Timestamp (UTC+07:00)'].apply(lambda row: row.day, 1)
